...
export class ChannelsService {
  ...
  async getChannelMessegesByChannelId(
    id: number,
    auth_user_id: string = '',
  ): Promise<GetChannelMessagesDto[]> {
    const user = await this.prisma.user.findUniqueOrThrow({
      where: { id: auth_user_id },
      include: { blocked: true, blocked_by: true, channels: true },
    });

    if (!user.channels.find((obj) => obj.channel_id === +id))
    ...
    messages = messages.filter(
      (obj) => !user_blocked_n_blocked_by.includes(obj.user.id),
    );
    return messages;
  }
  ...
  async getChannelUsersByChannelId(
    ...
    for (const user of channelUsers) {
      ...
      if (me.blocked_by.find((obj) => obj.blocked_by_id === user.user_id))
        continue;
      res.push(user);
    }
  ...
  async getChannelBannedUsers(
    id: number,
    auth_user_id: string = '',
  ): Promise<GetChannelBannedUsers[]> {
    const channelUser = await this.prisma.channelUsers.findUniqueOrThrow({
      where: { user_id_channel_id: { user_id: auth_user_id, channel_id: +id } },
    });
    if (channelUser.type === 'MEMBER')
      throw new HttpException(
        'MEMBER is not allowed to view channel banned users',
  ...
  async createChannel(
    ...
    if (body.type == 'PROTECTED' && !body.password)
      throw new HttpException(
        'PROTECTED password must have password',
        HttpStatus.BAD_REQUEST,
      );
    const created = await this.prisma.channels.create({
      data: {
        ...
        password:
          body.type == 'PROTECTED'
            ? await bcrypt.hash(body.password, await bcrypt.genSalt())
            : undefined,
        users: {
          create: [{ user_id: user.id, type: ChannelUserType.OWNER }],
        },
      },
      include: { users: true },
    });
  ...
  async createChannelMessages(
    ...
    if (channelUser.mute_time > new Date())
      throw new HttpException(
  ...
  async createChannelUsersByJoin(
    id: number,
    body: CreateChannelUsersDto,
    auth_user_id: string = '',
  ): Promise<GetChannelUsersDto> {
    const user =
      auth_user_id === '' // Testing
        ? await this.prisma.user.findFirst()
        : await this.prisma.user.findUnique({ where: { id: auth_user_id } });

    const channel = await this.prisma.channels.findUniqueOrThrow({
      where: { id: +id },
      include: { banned_users: true },
    });
    if (channel.banned_users.find((obj) => obj.user_id === user.id))
      throw new HttpException(
        'Banned user is not allowed to join',
        HttpStatus.BAD_REQUEST,
      );
    if (channel.type === 'PRIVATE')
      throw new HttpException('Channel is PRIVATE', HttpStatus.BAD_REQUEST);
      ...
      if (!body.password)
        throw new HttpException(
          'Channel requires password',
          HttpStatus.BAD_REQUEST,
        );
      else if (!(await bcrypt.compare(body.password, channel.password)))
        throw new HttpException(
          'Channel password incorrect',
  ...
  async createChannelUserByAdd(
    ...
    if (channelUser.type === 'MEMBER')
      throw new HttpException(
        'Request channel user is not Admin/Owner',
        HttpStatus.BAD_REQUEST,
      );
    ...
    if (user.banned_channels.find((obj) => obj.channel_id === +id))
      throw new HttpException(
  ...
  async createChannelBannedUsers(
    id: number,
    user_id: string,
    auth_user_id: string = '',
  ): Promise<GetChannelBannedUsers> {
    ...
    if (channelUser.type === 'MEMBER')
      throw new HttpException(
        'MEMBER cannot do ban operation',
        HttpStatus.BAD_REQUEST,
      );
    if (channelUser.user_id === user_id)
      throw new HttpException('Cannot ban yourself', HttpStatus.BAD_REQUEST);
    if (toBeBannedChannelUser)
      throw new HttpException(
        'Cannot ban existing member',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async updateChannelUser(
    ...
    const updatingChannelUser =
      await this.prisma.channelUsers.findUniqueOrThrow({
        where: { user_id_channel_id: { user_id: user_id, channel_id: +id } },
      });

    if (tmp_channelUser.type === 'MEMBER')
      throw new HttpException(
        'Member cannot alter other channel users',
        HttpStatus.BAD_REQUEST,
      );
    if (updatingChannelUser.type === 'OWNER')
      throw new HttpException(
        'Cannot alter OWNER channel user',
        HttpStatus.BAD_REQUEST,
      );
    if (body.type === 'OWNER')
      throw new HttpException(
        'Cannot set type to OWNER',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async deleteChannels(
    ...
    if (channelUser.type !== 'OWNER')
      throw new HttpException(
        'Must be an OWNER to delete channel',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async deleteChannelUsersByKick(
    id: number,
    user_id: string,
    auth_user_id: string = '',
  ): Promise<GetChannelUsersDto> {
    ...
    if (channelUser.type === 'MEMBER')
      throw new HttpException(
        'User is not Admin nor Owner',
        HttpStatus.BAD_REQUEST,
      );
    if (channelUser.user_id === toBeDeleted.user_id)
      throw new HttpException('Cannot kick yoursel', HttpStatus.BAD_REQUEST);
    if (channelUser.type === 'ADMIN' && toBeDeleted.type === 'OWNER')
      throw new HttpException('OWNER cannot be kicked', HttpStatus.BAD_REQUEST);

    await this.prisma.channelUsers.findUniqueOrThrow({
      where: {
        user_id_channel_id: { user_id: toBeDeleted.user_id, channel_id: +id },
      },
    });
  ...
  async deleteChannelUsersByLeave(
    id: number,
    auth_user_id: string = '',
  ): Promise<GetChannelsDto | GetChannelUsersDto> {
    const user =
      auth_user_id === ''
        ? await this.prisma.user.findFirst()
        : await this.prisma.user.findUniqueOrThrow({
    ...
    if (channelUser.type === 'OWNER') {
      ...
      await this.prisma.channelUsers.findUniqueOrThrow({
        where: {
          user_id_channel_id: { user_id: channelUser.user_id, channel_id: +id },
        },
      });
  ...
  async deleteChannelBannedUsers(
    id: number,
    user_id: string,
    auth_user_id: string = '',
  ): Promise<GetChannelBannedUsers> {
    ...
    if (channelUser.type === 'MEMBER')
      throw new HttpException(
        'MEMBER cannot unban user',
        HttpStatus.BAD_REQUEST,
      );

    await this.prisma.channelBannedUsers.findUniqueOrThrow({
      where: { user_id_channel_id: { user_id: user_id, channel_id: +id } },
    });
