...
export const GatewayChannelsMiddleware =
  (channelsService: ChannelsService, prisma: PrismaClient) =>
  async (socket: Socket, next: NextFunction) => {
    ...
    try {
      if (!(channelId = parseInt(socket.handshake.query.id as string)))
...
export const GatewayRootMiddleware =
  (prisma: PrismaClient) => async (socket: Socket, next: NextFunction) => {
    const logger = new Logger(GatewayChannelsMiddleware.name);
    let token: string;
...
export const GatewayChatsMiddleware =
  (userService: UserService, prisma: PrismaClient) =>
  async (socket: Socket, next: NextFunction) => {
    ...
    try {
      if (!(receiver_id = socket.handshake.query.receiver_id as string))
        throw new Error('Channel id is required');
      token = getCookieTokenFromWs(socket.handshake.headers.cookie);
      if (!token) throw new Error('Session cookie failed');
      const session = await prisma.session.findUniqueOrThrow({
