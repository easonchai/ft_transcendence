...
export class UserService {
	...
	async getAllUser(auth_user_id: string): Promise<User[]> {
		...
		users = users.filter((obj) => !friends.includes(obj.id) && !blocks.includes(obj.id) && obj.id !== auth_user_id);
	...
	async getUserById(user_id: string, auth_user_id: string): Promise<User> {
		...
		if (me.blocked.find((obj) => obj.blocked_id === user_id)) throw new HttpException('User is blocked', HttpStatus.BAD_REQUEST);
		if (me.blocked_by.find((obj) => obj.blocked_by_id === user_id)) throw new HttpException('User is blocked', HttpStatus.BAD_REQUEST);
		
		const user = await this.prisma.user.findUniqueOrThrow({ where: { id: user_id } });
		return user;
	}
	
	async getAcceptedUserFriends(user_id: string, auth_user_id: string): Promise<User[]> {
		const me = await this.prisma.user.findUniqueOrThrow({
			where: { id: auth_user_id },
			include: { blocked: true, blocked_by: true }
		})
	...
	async createUserFriends(user_id: string, auth_user_id: string): Promise<UserFriends> {
		if (user_id === auth_user_id) throw new HttpException('Cannot be friend with yourself', HttpStatus.BAD_REQUEST);
		...
		tba.blocked.forEach((obj) => {
			if (obj.blocked_id === auth_user_id) throw new HttpException('User is blocked cannot be friends', HttpStatus.BAD_REQUEST);
		})
		tba.blocked_by.forEach((obj) => {
			if (obj.blocked_by_id === auth_user_id) throw new HttpException('User is blocked cannot be friends', HttpStatus. BAD_REQUEST);
		})
	...
	async createUerBlocks(user_id: string, auth_user_id: string): Promise<UserBlocks> {
		...
		tba.friends.forEach((obj) => {
			if (obj.friend_id === auth_user_id) throw new HttpException('Friends cannot be blocked', HttpStatus.BAD_REQUEST);
		})
		tba.friend_of.forEach((obj) => {
			if (obj.user_id === auth_user_id) throw new HttpException('Friends cannot be blocked', HttpStatus.BAD_REQUEST);
		})
	...
	async createUserMessages(user_id: string, body: CreateChannelMessagesDto, auth_user_id: string): Promise<UserMessages> {
		if (user_id === auth_user_id) throw new HttpException('Cannot talk to yourself', HttpStatus.BAD_REQUEST);
		...
		if(isBlock) throw new HttpException('Cannot chat with blocked user', HttpStatus.BAD_REQUEST);
	...
	async updateUserFriends(user_id: string, body: UpdateUserFriendsDto, auth_user_id: string): Promise<UserFriends> {
		const tba = await this.prisma.user.findUniqueOrThrow({ where: { id: user_id } });
	...
	async deleteUserFriend(user_id: string, auth_user_id: string): Promise<UserFriends> {
		let deleted: UserFriends;
		
		try {
			deleted = await this.prisma.userFriends.delete({
				where: { user_id_friend_id: { user_id: auth_user_id, friend_id: user_id } }
			})
		} catch (e: any) {
			deleted = await this.prisma.userFriends.delete({
				where: { user_id_friend_id: { user_id: user_id, friend_id: auth_user_id } }
	...
	async deleteUserBlock(user_id: string, auth_user_id: string): Promise<UserBlocks> {
		let deleted: UserBlocks;
		
		deleted = await this.prisma.userBlocks.delete({
			where: { blocked_by_id_blocked_id: { blocked_by_id: auth_user_id, blocked_id: user_id } }
		})
