      ...
      <ProCard bordered style={{ overflowY: "scroll" }}>
        <div className="h-[60vh]">
          {chats.map((item, index) => {
            const rowProps =
              index === props.chats.length - 1 ? { ref: messageRef } : {};
            const user = users?.find((obj) => obj.id === item.sender_id);
            const found = props.blockedUsers.find(
                ...
                <Col flex={"auto"} className="text-right">
                  <div>
                    <Typography.Text type="secondary">
                      {user?.name} - {moment(item.created_at).format("LLL")}
                    </Typography.Text>
                  </div>
                  {item.message}
                </Col>
      ...
      <ProCard bordered style={{ overflowY: "scroll" }}>
        <div className="h-[60vh]">
          {chats.map((item, index) => {
            const rowProps =
              index === props.chats.length - 1 ? { ref: messageRef } : {};
            const channeluser = users?.find(
              (obj) => obj.user_id === item.user_id
            )?.user;
            const found = props.blockedUsers.find(
              (obj) => obj.id === item.user_id
            );
            if (found) return null;
