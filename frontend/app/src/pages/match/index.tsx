...
import { KeyboardEvent, RefObject, useEffect, useLayoutEffect, useRef, useState } from 'react';
...
const useSize = (target: RefObject<HTMLDivElement>) => {
	...
	useLayoutEffect(() => {
		setSize({height: target.current!.getBoundingClientRect()!.width * 0.5256, width: target.current!.getBoundingClientRect()!.width - 48})
	}, [target]);
	useResizeObserver(target, (entry) => setSize({height: entry.contentRect.width * 0.5256, width: entry.contentRect.width - 48}));
	return size;
}
