...
const index = () => {
  ...
  const [body, setBody] = useState<CreateChannelsRequest>({
    name: "",
    type: "PUBLIC",
    password: "",
  });
  const [createChannelModalIsOpen, setCreatechannelModalIsOpen] =
  ...
  const {
    isLoading: getMyChannelsIsLoading,
    isSuccess: getMyChannelsIsSuccess,
  } = useQuery({
  ...
  const joinChannelsMutation = useMutation({
    ...
    onSuccess: (res) => {
      router.push({
        pathname: `/channels/${res.channel_id}`,
        query: { join: 1 },
      });
    },
    onError: (e: any) => {
      messageApi.error({ content: e.message ?? "Failed to join channel" });
    },
  });
  ...
  const passwordMutation = useMutation({
    mutationFn: (id: number) =>
      channelsService.joinChannels(id, { password: password }),
    onSuccess: (res) => {
      router.push({
        pathname: `/channels/${res.channel_id}`,
        query: { join: 1 },
  ...
  useEffect(() => {
    if (router.query.leave === "1")
      messageApi.success("Successfuly leave group");
  }, [router.isReady]);
  ...
  return (
    <>
      ...
      <PageContainer
        extra={[
          <Button
            key="1"
            icon={<PlusOutlined />}
            onClick={() => setCreatechannelModalIsOpen(true)}
          >
            Create
          </Button>,
        ]}
              ...
              renderItem={(item, index) => (
                <List.Item
                  ...
                  actions={[
                    <Button
                      key={index}
                      onClick={() => router.push(`/channels/${item.id}`)}
                    >
                      Chat
                    </Button>,
                  ]}
              ...
              renderItem={(item, index) => (
                <List.Item
                  ...
                  actions={[
                    item.type !== "PROTECTED" ? (
                      <Button
                        onClick={() => joinChannelsMutation.mutate(item.id)}
                      >
                        Join
                      </Button>
                    ) : (
      ...
      <CreateChannelModal
        {...{
          createChannelModalIsOpen,
          setCreatechannelModalIsOpen,
          body,
          setBody,
...
const CreateChannelModal = (props: CreateChannelModalProps) => {
  ...
  return (
    <Modal
      ...
      onOk={() => form.submit()}
      onCancel={() => {
        props.setCreatechannelModalIsOpen(false);
        form.resetFields();
      }}
      ...
      >
        <Form.Item name="name" label="Name" rules={[{ required: true }]}>
          <Input
            ...
            onChange={(e) =>
              props.setBody({ ...props.body, name: e.target.value })
            }
          />
        </Form.Item>
        <Form.Item
...
interface PasswordModalProps {
  ...
  passwordMutation: UseMutationResult<
    ChannelUsersResponse,
    any,
    number,
    unknown
  ...
  return (
    ...
    >
      <Form
        ...
        onFinish={(data) => {
          console.log(data);
          props.passwordMutation.mutate(props.passwordId);
          form.resetFields();
        }}
      >
        <Form.Item
